...
import cn.icebg.hospital.mapper.PowerRoleMapper;
import cn.icebg.hospital.mapper.PowerRoleMenuRelationMapper;
import cn.icebg.hospital.mapper.PowerRoleResourceRelationMapper;
import cn.icebg.hospital.mapper.dao.PowerRoleDao;
import cn.icebg.hospital.service.IPowerRoleService;
import cn.icebg.hospital.entity.*;
...
public class PowerRoleServiceImpl implements IPowerRoleService {
    ...
    public boolean countByOpenid(String openid) {
        return powerAccountMapper.countByOpenid(openid) > 0;
    }

    @Override
    public PowerAccount registerByOpenid(String openid, String sessionKey) {
        PowerAccount newAccount = new PowerAccount();
        newAccount.setOpenid(openid);
//        newAccount.setSessionKey(sessionKey);  // 如果需要存储 sessionKey
        newAccount.setName("微信用户" + openid.substring(0, 5));  // 生成默认用户名
        powerAccountMapper.insert(newAccount);
        return newAccount;
    }

    @Override
    public Optional<PowerAccount> loginByOpenid(String openid) {
        return Optional.ofNullable(powerAccountMapper.selectByOpenid(openid));
    }
