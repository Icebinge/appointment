...
import cn.icebg.hospital.common.security.AccountDetails;
import cn.icebg.hospital.dto.param.PowerAccountRegisterParam;
import cn.icebg.hospital.dto.param.PowerAccountStatusParam;
import cn.icebg.hospital.dto.param.UserRegisterParam;
import cn.icebg.hospital.entity.*;
import cn.icebg.hospital.mapper.PowerAccountMapper;
import cn.icebg.hospital.mapper.PowerAccountRoleRelationMapper;
import cn.icebg.hospital.service.ILogAccountLoginService;
import cn.icebg.hospital.service.IPowerAccountService;
import cn.icebg.hospital.service.IUserBasicInfoService;
import cn.icebg.hospital.common.security.JwtTokenUtil;
import cn.icebg.hospital.mapper.dao.PowerAccountRoleRelationDao;
...
//        account.setPassword(param.getPassword());
     ...
     */
    @Override
    public Optional<String> login2(String name, String password) {
        ...
        try {
            ...
            // 密码不正确
            if (!powerAccountService.checkPassword2(account.getId(), password)) {
     ...
     */
    @Override
    public Optional<String> weChatLogin(String openId) {
        ...
        try {
            ...
            // 这里需要记录登录时间
            updateLoginTimeByOpenId(openId);
            logAccountLoginService.insertByOpenId(openId);

            // 注意中间用空格隔开
            jwt = tokenHead + BLANK_SPACE + token;

        } catch (AuthenticationException e) {
            LOGGER.warn("user :{} , login fail :{}", openId, e.getMessage());
        }
     ...
     */
    @Override
    public boolean checkPassword2(Long id, String password) {

        PowerAccountExample example = new PowerAccountExample();
        example.createCriteria()
                .andIdEqualTo(id)
                .andPasswordEqualTo(password);
        PowerAccount account = accountMapper.selectByExample(example).get(0);
...
//        account.setPassword(password);
     ...
     */
    @Override
    public void updateLoginTimeByOpenId(String openId) {
        Optional<PowerAccount> accountOptional = getByOpenId(openId);
