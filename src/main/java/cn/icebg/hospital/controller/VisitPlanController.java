...
public class VisitPlanController {
    ...
    @ApiOperation(value = "根据医生编号，获取出诊信息", notes = "传入 医生编号")
    @RequestMapping(value = "/plan/doctor/undate", method = RequestMethod.GET)
    public CommonResult<List<VisitPlanResiduesDTO>> searchVisitPlanByDoctorUndate(@RequestParam Long doctorId) {

        if (!hospitalDoctorService.count(doctorId)) {
            return CommonResult.validateFailed("不存在，该医生编号！");
        }

        return CommonResult.success(planService.getDoctorPlanByDoctor(doctorId));
    }
}
